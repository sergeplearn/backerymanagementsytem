...
<table class="table">
  ...
  <tbody>
    @foreach($houserental as $houserental)
   
    <tr>
<td>no</td>
<td>{{ $houserental->fname}} </td>
<td>{{ $houserental->house }}</td>
<td>{{ $houserental->price }}</td>
<td><a href="/houserent/rent/{{ $houserental->house }}">rent</a></td>
<td><?php


$d=(strtotime($date) - strtotime( $houserental->date ));
 $month = 60*60*24*7*4;
$total =ceil($d/$month);
echo $total;
?></td>
<td>
<a href="/houserent/edit/{{ $houserental->info }}">info</a>
</td>
<td>

<a href="/houserent/payment/{{ $houserental->info }}">info</a>
</td>
    </tr>
    @endforeach
  </tbody>
